...
def main():
    ...
    for value in os.listdir(front_path_base):
        ...
        large_img_topview = np.zeros((topview_height, front_img.shape[1]+topview_width, 3), dtype=np.uint8)
        ...
        large_img_topview[crop_h_max:,0:int(front_img.shape[1]*0.5),:] = left_resize_pic
        large_img_topview[crop_h_max:,int(front_img.shape[1]*0.5):int(front_img.shape[1]*0.5*2), :] = right_resize_pic
        ...
        topview = cv2.resize(src=topview,
                            dsize=(int(topview_width),
                                int(topview_height)))
        large_img_topview[:,front_img.shape[1]:,:] = topview
