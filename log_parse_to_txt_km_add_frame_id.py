...
def front_rear_img_log(line):
    ...
    front_phy_x = float(list_num[2]) 
    front_phy_y = float(list_num[3])
    rear_phy_x = float(list_num[4]) 
    rear_phy_y = float(list_num[5])

    front_img_x = float(list_num[8]) 
    front_img_y = float(list_num[9])

    rear_img_x = float(list_num[10]) 
    rear_img_y = float(list_num[11])

  
    return frame_id, front_img_x, front_img_y, rear_img_x, rear_img_y 

def front_rear_img_log_bak(line):
...
def front_rear_phy_log(line):
    ...
    front_phy_x = float(list_num[2]) # delete [
    front_phy_y = float(list_num[3])
    rear_phy_x = float(list_num[4]) # delete [
    rear_phy_y = float(list_num[5])

    front_img_x = float(list_num[8]) # delete [
    front_img_y = float(list_num[9])

    rear_img_x = float(list_num[10]) # delete [
    rear_img_y = float(list_num[11])
  
    return frame_id, front_phy_x, front_phy_y, rear_phy_x, rear_phy_y

def front_rear_phy_log_bak(line):
...
def log_process(file_path_all, channel_id):
        ...
        with open(file_path_all, 'r') as file_read:
                while True:
                        ...
                        #         continue
                        if front_phy_x == 0 and front_phy_y == 0:
                                continue
                        ...
                        if(frame_id in log_data_img_dict):
                                if(first_flag):
                                        frame_id_int = int(frame_id) + count
                                        frame_id_tmp = frame_id_int
                                        count = count + 1 
                                        first_flag = False
...
if __name__ == "__main__":
        ...
        # src_log_path = "/home/xieyi/pic/zadas_tjp_01_data/tjp_OBDcar_capture1.log"
        # img_csv_path = "/home/xieyi/pic/zadas_tjp_01_data/capture1_img.csv"
        # phy_csv_path = "/home/xieyi/pic/zadas_tjp_01_data/capture1_phy.csv"

        src_log_path = "/home/xieyi/pic/zadas_tjp_01_data/tjp_KMcar_capture1.log"
        img_csv_path = "/home/xieyi/pic/zadas_tjp_01_data/capture1_img_km.csv"
        phy_csv_path = "/home/xieyi/pic/zadas_tjp_01_data/capture1_phy_km.csv"
        channel_id = "front"
